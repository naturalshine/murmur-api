...
async function main() {
    ...
    const mainName = 'videos_80001_5772'
	...
	const VideoNFT = await ethers.getContractFactory("Video")
	...
	const videoNFT = await VideoNFT.deploy(tablelandBaseURI, mainName)
	...
	await videoNFT.deployTransaction.wait(5)

	// Log the deployed address and call the getter on `baseURIString` (for demonstration purposes)
	console.log(`\nVideoNFT contract deployed on ${network.name} at: ${videoNFT.address}`)
	const baseURI = await VideoNFT.baseURIString()
	console.log(`VideoNFT is using baseURI: ${baseURI}`)
