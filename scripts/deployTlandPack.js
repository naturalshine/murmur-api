...
async function main() {
    ...
    const mainName = 'packs_80001_5721'
	...
	const PackNFT = await ethers.getContractFactory("Pack")
	...
	const packNFT = await PackNFT.deploy(tablelandBaseURI, mainName)
	...
	await packNFT.deployTransaction.wait(5)

	// Log the deployed address and call the getter on `baseURIString` (for demonstration purposes)
	console.log(`\nPackNFT contract deployed on ${network.name} at: ${packNFT.address}`)
